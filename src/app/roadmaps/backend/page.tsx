...
export default function BackendRoadmapPage() {
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem("roadmap-backend-progress");
    if (savedProgress) {
      try {
  ...
  useEffect(() => {
    if (isLoaded && completedWeeks.length === backendRoadmap.modules.length) {
      const hasCelebrated = sessionStorage.getItem("roadmap-backend-celebrated");
      if (!hasCelebrated) {
        setIsCelebrationOpen(true);
        sessionStorage.setItem("roadmap-backend-celebrated", "true");
      }
    }
  ...
  useEffect(() => {
    if (isLoaded) {
      localStorage.setItem("roadmap-backend-progress", JSON.stringify(completedWeeks));
    }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-brand-primary/30">
      ...
      <RoadmapHero 
        ...
        totalCount={backendRoadmap.modules.length}
      />

      <TimelineProgress 
        totalWeeks={backendRoadmap.modules.length}
      ...
      <section className="max-w-5xl mx-auto px-6 pb-32">
        ...
        >
          <div className="flex items-center justify-between mb-12">
            <h2 className="text-3xl font-bold tracking-tight">Backend Learning Roadmap</h2>
            
          </div>

          <div className="space-y-6">
            {backendRoadmap.modules.map((module) => (
              <ModuleAccordion 
                key={module.id}
                module={module}
